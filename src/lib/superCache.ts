...
export const superCache = {
  ...
  generations: () => singleTag('generations'),
  userGenerations: ({ userId }: { userId: string }) =>
    singleTag(`userGenerations:${userId}`, ['generations']),
  generation: ({ id }: { id: string }) =>
    singleTag(`generation:${id}`, ['generations']),
}
